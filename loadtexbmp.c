...
unsigned int LoadTexBMP(const char* file)
{
   ...
   if (dx<1 || dx>max) printf("%s image width %d out of range 1-%d\n",file,dx,max);
   if (dy<1 || dy>max) printf("%s image height %d out of range 1-%d\n",file,dy,max);
   ...
   if (fseek(f,off,SEEK_SET) || fread(image,size,1,f)!=1) printf("Error reading data from image %s\n",file);
   fclose(f);
